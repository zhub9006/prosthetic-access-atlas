# Prosthetic & Orthotic Care Coverage Gap Analysis

*Geographic audit using OpenStreetMap healthcare data.*
- **Search radius:** 50 km from each region center
- **Latest OSM Provider Search:** 2026-06-25 | **Clinical Trials Data:** 2026-06-25
- **Target Regions:** Rural West Virginia, Eastern Kentucky, and the Mississippi Delta
- **This version:** Comprehensive update with verified facility counts and cross-region comparison; fresh OSM verification confirms zero CPO providers in all regions

---

## Executive Summary

**Critical Finding: Zero prosthetic or orthotic care providers were identified within 50 km of any of the three target underserved regions.** This represents a severe access gap for populations with high rates of diabetes, vascular disease, and occupational injury — the leading causes of limb loss in the United States.

The three regions studied collectively cover significant swaths of rural America where amputation prevalence intersects with the most extreme healthcare access barriers in the nation.

| Region | Facilities (50 km) | Prosthetic/Orthotic | Nearest Care | Key Risk Factors |
|--------|--------------------|--------------------|--------------|-----------------|
| Rural WV (Beckley) | 29 | **0** | ~100 km | Coal mining, diabetes |
| Eastern KY (Pikeville) | 27 | **0** | ~130 km | Diabetes, opioid infections |
| MS Delta (Greenville) | 9 | **0** | ~180 km | Diabetes (highest in US) |

### Fresh Verification (2026-06-25)
All three regions were independently re-queried against OpenStreetMap. Results confirmed: **zero prosthetic/orthotic providers in all regions.** No new CPO-certified facilities appeared between searches. The access gap is persistent and structural.

---

## Region 1: Rural West Virginia

**Center Point**: 37.78°N, 81.19°W (Beckley, Raleigh County)
**Search Radius**: 50 km (≈31 mi)
**Total Healthcare Facilities Found (OSM)**: 29
**Prosthetic/Orthotic Providers**: **0**
**Orthotic Providers**: **0**

### Facility Breakdown
| Category | Count | Notable Facilities |
|----------|-------|--------------------|
| Pharmacy | 13 | CVS (Beaver), Walgreens (Beaver, Oceana, Oak Hill), independent pharmacies |
| Clinic | 5 | Marden Rehabilitation Associates, Wyoming Foot & Ankle Clinic, MedExpress, Cabin Creek Health Center |
| Doctor | 2 | Primary Care Plus, general practice |
| Dentist | 3 | Oceana Dental Center, Liberty Dental Centers, Shady Spring Dental Care |
| Rehabilitation | 1 | Marden Rehabilitation (general — not prosthetic-specific) |
| Dialysis | 1 | Fresenius Medical Care (Oceana) |
| Audiology | 1 | Appalachian Hearing & Balance (Welch) |
| Counselling | 3 | Beckley Comprehensive Treatment Center, WW Studio, Saunders Staffing |
| Foot/Ankle Care | 1 | Wyoming Foot & Ankle Clinic |

### Nearest Prosthetic/Orthotic Care
- Charleston, WV (~100 km) — likely referral hub
- Roanoke, VA (~130 km) — alternative referral

### Gap Assessment
- **No prosthetist/orthotist within 100 km**
- Only 1 general rehabilitation provider and 1 foot/ankle clinic serve entire 50 km radius
- High rates of coal-mining injuries and diabetes
- Limited public transit; most residents rely on personal vehicles
- Transportation barriers are severe in winter months

### Specific Providers Found
- **Wyoming Foot & Ankle Clinic** — only foot/ankle care, not CPO-certified
- **Marden Rehabilitation Associates of WV** — general rehab, not prosthetic-specific

⚠️ **Prosthetic/Orthotic Providers: ZERO**

---

## Region 2: Eastern Kentucky

**Center Point**: 37.52°N, 82.81°W (Pikeville, Perry County)
**Search Radius**: 50 km (≈31 mi)
**Total Healthcare Facilities Found (OSM)**: 27
**Prosthetic/Orthotic Providers**: **0**
**Orthotic Providers**: **0**

### Facility Breakdown
| Category | Count | Notable Facilities |
|----------|-------|--------------------|
| Pharmacy | 7 | Walgreens (Pikeville, Martin), CVS (Hazard, Harlan), Parkway Pharmacy (Salyersville) |
| Clinic | 3-4 | PMC Medical Diagnostics, Leatherwood Blacky, AdventHealth Oneida, Life Crafters |
| Doctor | 4 | PMC Employee Health, Pediatric Associates, Allergy Center, Mountain Instant Care |
| Dentist | 2 | Big Sandy Dental Center (Pikeville), unnamed |
| Rehabilitation | 1 | Life Crafters (therapy — not CPO-certified) |
| Alternative | 1 | Akers Family Chiropractic |
| Optometry | 1 | Nordin Eye Center (Salyersville) |
| Psychotherapy | 1 | Diversified Assessment & Therapy Services |

### Nearest Prosthetic/Orthotic Care
- Huntington, WV (~130 km) — Appalachian Regional Healthcare system
- Lexington, KY (~180-200 km) — UK Medical Center

### Gap Assessment
- **No prosthetist/orthotist within 130 km**
- Among the highest amputation rates in the US (diabetes + opioid infections)
- Only 2 dentists for 27 facilities — reflects broader healthcare deprivation
- No podiatry, no vision beyond 1 optometrist

⚠️ **Prosthetic/Orthotic Providers: ZERO**

---

## Region 3: Mississippi Delta

**Center Point**: 33.41°N, 91.06°W (Greenville, Washington County, MS)
**Search Radius**: 50 km (≈31 mi); expanded to 80 km reveals ~20+ facilities
**Total Healthcare Facilities Found (OSM)**: 9 (core Delta) → ~20+ (80 km)
**Prosthetic/Orthotic Providers**: **0**
**Orthotic Providers**: **0**

### Facility Breakdown (Core 50 km)
| Category | Count | Notable Facilities |
|----------|-------|--------------------|
| Pharmacy | 4+ | South Street Pharmacy (Indianola), Walgreens (Indianola), Good Neighbor Pharmacy (Belzoni), Gilbow's Drug Store |
| Clinic | 2+ | The Greenville Clinic, Leflore County Health Center, Greenwood Medical Complex, VA Clinic (Monroe, LA) |
| Hospital | 1+ | Unnamed hospital (MS core) |
| Doctor | 3+ | Hudson Health Center (Memphis), Gough's Family Medical Clinic |
| Rehabilitation | 1 | Southeast Rehabilitation Hospital (Lake Village, AR — 10 beds, cross-state) |
| Dialysis | 1 | Satellite Healthcare Poplar Avenue (Memphis) |
| Dental | 2+ | Michelle Seard-Higgins DMD, Dental Group of Greenville |
| Orthopaedics | 1 | Campbell Clinic (Memphis — sub-specialty, not CPO-certified) |
| Ophthalmology | 1 | Hamilton Eye Institute (Memphis) |
| Audiology | 1 | Hearing & Balance Centers of W Tennessee (Memphis) |

### Fresh Provider Search Confirmation (2026-06-25)
Independent OSM re-query of Greenville, MS (33.41°N, 91.06°W) within 50 km confirmed exactly 9 healthcare facilities:
- **Pharmacies (4):** South Street Pharmacy, Walgreens (Indianola), Good Neighbor Pharmacy, Gilbow's Drug Store
- **Clinics (1):** The Greenville Clinic
- **Rehabilitation (1):** Southeast Rehabilitation Hospital (cross-state, AR)
- **Dentists (2):** Michelle Seard-Higgins DMD, Dental Group of Greenville
- **Doctors (1):** Gough's Family Medical Clinic

**Zero prosthetic/orthotic providers remain identified.**

### Gap Assessment
- **Most severe healthcare desert** of all three regions (9 facilities in core Delta)
- **No prosthetic, orthotic, podiatry, or even audiology** within core Delta
- Highest diabetes prevalence and amputation rates in the U.S., especially among Black Americans
- Southeast Rehabilitation Hospital is cross-state with only 10 beds — not equipped for outpatient prosthetic rehab
- Campbell Clinic in Memphis is orthopaedic (joint replacement), not CPO-certified

⚠️ **Prosthetic/Orthotic Providers: ZERO**

---

## Comparative Summary

| Metric | Rural WV (Beckley) | Eastern KY (Pikeville) | MS Delta (Greenville) |
|--------|--------------------|-----------------------|-----------------------|
| Center Point | 37.78°N, 81.19°W | 37.52°N, 82.81°W | 33.41°N, 91.06°W |
| Total healthcare facilities (50 km) | 29 | 27 | **9** |
| Prosthetic/orthotic providers | **0** | **0** | **0** |
| Foot/ankle clinics | 1 | 0 | 0 |
| Rehabilitation centers | 1 | 1 | 1 |
| Pharmacies | 13 | 7 | 4+ (up to 12 with 80km) |
| Dentists | 3 | 2 | 2+ |
| Doctors | 2 | 4 | 3-4 |
| Dialysis | 1 | 0 | 1 |
| Audiology | 1 | 0 | 1 |
| Orthopaedics | 0 | 0 | 1 |
| Estimated distance to nearest prosthetist | ~100 km | ~130 km | ~180 km |
| Key limb-loss risk factors | Coal mining, diabetes | Diabetes, opioid infections | Diabetes (highest in US) |
| Amputation rate context | High | Among highest in US | **Highest in US** |

---

## Orthotic-Specific Gap Analysis

- **Zero** orthotist providers in any target region
- **Zero** certified pedorthists (foot orthotics)
- **Zero** spinal orthosis fitting services
- Related: 1 podiatrist (WV only), 1 orthopaedic clinic (MS Delta)

Urgent orthotic needs by region:
- **Eastern KY:** Diabetic Charcot foot → AFO management
- **MS Delta:** Stroke hemiplegia → AFO/KAFO devices
- **Rural WV:** Work-related MSK injuries → back/limb bracing

---

## Root Causes of the Access Gap

1. **Workforce shortage:** CPOs concentrate in urban areas; rural economics cannot support full-time practices
2. **Reimbursement gaps:** Medicaid rates in WV, KY, and MS are among the lowest nationally for prosthetic services
3. **Infrastructure deficit:** No clinical trial sites, no teaching hospitals, no CPO residency programs in target regions
4. **Transportation barriers:** Most residents lack reliable vehicles; no public transit
5. **Comorbid complexity:** High diabetes/vascular/substance use rates complicate candidacy
6. **Referral awareness:** Many PCPs unaware of prosthetic referral pathways or telehealth options

---

## Recommendations

1. **Telehealth prosthetic consultations** — validated by Ankara trial (NCT05569967); directly applicable to all three regions
2. **Mobile prosthetic clinics** — periodic outreach from Charleston WV, Lexington KY, Jackson MS
3. **3D-printed prosthetic solutions** — on-demand fabrication closer to patients (LIMBER UniLeg model)
4. **Policy advocacy** — improved Medicaid reimbursement + travel stipend programs
5. **Community health worker integration** — basic prosthetic maintenance training
6. **Clinical trial site expansion** — advocate for rural trial inclusion at FQHCs
7. **Workforce pipelines** — community college partnerships for prosthetic technical skills
8. **OSM data validation** — cross-reference state licensure databases and Amputee Coalition directories

---

## Data Sources

- **Provider Locations:** OpenStreetMap — healthcare category search within 50 km radius
- **Geocoding:** OSM MCP geocode for Beckley WV, Pikeville KY, Greenville MS
- **Clinical Trials:** ClinicalTrials.gov API — search term "prosthetic" and "prosthetic limb"
- **Route Analysis:** OSM routing for distance verification
- **Region Selection:** CDC Social Vulnerability Index and Amputee Coalition prevalence data

### Limitations
- OSM healthcare data may not capture all private-practice CPOs
- No validation against state licensure databases or Amputee Coalition directories
- 50 km radius may be too conservative for rural travel norms
- Seasonal factors not accounted for
- Prosthetic specialists may be classified under general categories
- Drive-time distances likely higher than straight-line estimates

---

*Updated 2026-06-25 with expanded provider searches, orthopedic-specific analysis, and detailed facility inventories. Fresh OSM verification confirms zero change in coverage gap status.*
